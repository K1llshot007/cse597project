...
tf.app.flags.DEFINE_string('image_folder', 
                                '/scratch/abhinav597project/RAT-Diffusion/samples/DiT-L-2-0028000-size-256-vae-ema-cfg-1.5-seed-100',
...
def load_data(fullpath):
    print("Searching for images in:", fullpath)
    images = []
    for path, subdirs, files in os.walk(fullpath):
        for name in files:
            if name.lower().endswith(('jpg', 'png')):  # Support both JPG and PNG in any case
    ...
    if not images:
        ...
        print(f"Loaded {len(images)} images. Example size: {images[0].size}")
    return images
